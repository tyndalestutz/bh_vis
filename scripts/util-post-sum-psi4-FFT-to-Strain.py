...
def frequency_cutoff_quad_fit(
    time, omega, t_start=200, t_end=400
):  # Revisit/Room to investigate and improve.
...
def psi4_fft_to_strain(
    ...
    sY = D.sYlm(
        s, quaternionic.array.from_spherical_coordinates(colatitude, azimuth_values)
    )
    ...
    for azi_idx, azi in enumerate(azimuth_values):
        ...
        with open(output_file, "w", encoding="utf-8") as file:
            ...
            for t, real, imag in zip(
                time, np.real(strain[:, azi_idx]), np.imag(strain[:, azi_idx])
            ):
