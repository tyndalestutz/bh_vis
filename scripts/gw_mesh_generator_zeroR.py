...
output_directory = "/home/guest/Documents/BH_Vis_local/data/mesh/gw_test6/"
...
def set_sph_harm_array(l, m, s):
    ...
    for j in range(resolution):
        ...
        for i in range(resolution):
            ...
            # Skip points below the radius of 2
            if current_r < 2:
                continue
...
for current_time in h_time:
    ...
    writer.SetFileName(f"{output_directory}/mesh-{state:04d}.vts")
    writer.SetInputData(grid)
    writer.Write()

end_time = time.time()
print(f"\nFinished creating {length} meshes in {end_time - start_time:.2f} seconds")